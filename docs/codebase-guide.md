...
| MVP / Win / Loss / Author bonuses | `ai/verdict.logic.ts` `MVP_BONUS` / `WIN_BONUS` / `LOSS_PENALTY` / `AUTHOR_BONUS` | `verdict.logic.test.ts`; `VerdictBanner.tsx`; `MotionForm.tsx`; **`SideLockConfirm.tsx`** for the loss penalty (§19 needs it before *and* after); `/rules` rule 6 |
