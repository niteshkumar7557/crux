...
- **§8 remaining (not built):** the **hot-extension** (§8.1, "§8-completion Slice 3") and **feed/domain card** countdown+concluded states ("Slice 4"). Both designed in the §8-completion spec above; migration column `hot_extended` for the hot-extension is **already applied** (Slice 6's migration). Plans for these two not yet written.
- All six slices verified (backend: vitest + real conclusion/analyst runs; frontend: tsc + lint clean, real OG renders eyeballed). **Nothing is committed** — commit messages were handed over per slice.
