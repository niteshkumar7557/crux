...
- **§8 deferred (not built):** the **hot-extension** (§8.1), the losing-side **standout** nod (§8.3), **feed/domain card** countdown+concluded states, and §8.5 shift #3 (per-user diminishing returns).
- All four slices verified (backend: vitest + real conclusion/analyst runs; frontend: tsc + lint clean, Playwright 0 console errors). **Nothing is committed** — commit messages were handed over per slice.
...
- **External AI: defend by design, never by detection.** No AI-text detector — they're unreliable, an arms race, and they false-flag non-native English speakers. Instead, three scoring shifts make pasting generic AI output a *losing* strategy:
  ...
  3. **Deflate the ratchet** — diminishing returns on repeated comments in one debate; weight comments that **survive into the winning analysis or are credited in the verdict** higher. *(Deferred.)*
