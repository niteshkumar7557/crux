...
// Presets are static SVGs baked into the image and served by express.static —
// shared, tiny, and regenerated by a rebuild, so they need no durable storage.
// Uploads do, and live in object storage: see lib/avatarStorage.ts. This prefix
// is only the local-disk fallback used in development.
...
// There are exactly two kinds of avatar: a shared preset, or this user's own
// upload. Testing for "not a preset" rather than for a particular upload prefix
// keeps this correct whether the upload lives in object storage (an absolute
// URL) or on the local disk in development (a relative path).
